...
class StepObserver:
    ...
    def after(self, optimizer, args, kwargs):
        ...
        # get() is essential: indexing the defaultdict creates empty entries for
        # original A's parameters that have no gradient, mutating state_dict.
        state = [optimizer.state[p] for _, p in self.named if 'exp_avg_sq' in optimizer.state.get(p,{})]
...
def optimizer_diagnostics(template,cfg,records,info,device,source,out,modes=('D0','D1','D2')):
    ...
    for mode in modes:
        ...
        if mode!='D2':
            populated=[s for s in after['state'].values() if s]
            assert populated and all(float(s['step'])==600 for s in populated)
