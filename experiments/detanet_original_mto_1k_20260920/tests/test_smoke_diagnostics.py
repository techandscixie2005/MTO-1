...
from smoke_diagnostics import StepObserver, intervene_optimizer, svd_solve, loss_metrics, cpu_tree
...
class DiagnosticContracts(unittest.TestCase):
    ...
    def test_loaded_optimizer_source_isolation(self):
        p=torch.nn.Parameter(torch.tensor([2.,-1.]))
        source_opt=torch.optim.Adam([p],lr=.001,amsgrad=True)
        p.square().sum().backward();source_opt.step()
        source=cpu_tree(source_opt.state_dict())
        expected=cpu_tree(source)
        for _ in range(2):
            clone=torch.nn.Parameter(p.detach().clone())
            opt=torch.optim.Adam([clone],lr=.001,amsgrad=True)
            opt.load_state_dict(cpu_tree(source))
            clone.square().sum().backward();opt.step()
            for key,value in source['state'][0].items():self.assertTrue(torch.equal(value,expected['state'][0][key]))
            self.assertEqual(float(opt.state[clone]['step']),2.)

    def test_observer_leaves_unused_parameter_state_absent(self):
