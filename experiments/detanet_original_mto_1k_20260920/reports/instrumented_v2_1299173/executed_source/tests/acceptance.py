...
class Acceptance(unittest.TestCase):
    ...
    def test_03_graph_operator_then_graph_builder(self):
        ...
        # cdist's GEMM implementation can produce nonzero diagonal distances in
        # FP32. Self-loop exclusion is an index property, never a d>0 heuristic.
        legacy=torch.cdist(pos,pos)
        d=torch.linalg.vector_norm(pos[:,None]-pos[None,:],dim=-1)
        mask=(d<5)&~torch.eye(len(pos),dtype=torch.bool,device=DEVICE)&(batch[:,None]==batch[None,:])
        old_mask=(legacy<5)&(legacy>0)&(batch[:,None]==batch[None,:])
        diff=old_mask^mask
        MEASUREMENTS['graph/cdist_diagonal_max']=float(legacy.diagonal().max())
        MEASUREMENTS['graph/cdist_reference_difference']=diff.nonzero().tolist()
        if diff.any():
            indices=diff.nonzero()
            assert (indices[:,0]==indices[:,1]).all(), 'Unexpected off-diagonal graph discrepancy'
