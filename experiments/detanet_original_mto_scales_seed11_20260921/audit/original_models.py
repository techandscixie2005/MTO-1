...
class SpectrumModel(nn.Module):
    def __init__(self, cfg, n_ref, grid, variant):
        ...
        if variant != VARIANTS[0]:
            self.mto = MolecularTensorOrbitals(cfg, n_ref, variant == VARIANTS[2])
...
def build_triplet(cfg, n_ref, grid, seed):
    ...
    c = SpectrumModel(cfg, n_ref, grid, VARIANTS[2])
    ...
    return dict(zip(VARIANTS, (a, b, c)))
